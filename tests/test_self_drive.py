...
def _session(**kwargs: Any) -> self_drive.SelfDriveSession:
    return self_drive.SelfDriveSession(
        session_id="s1", goal="Öffne die Einstellungen", **kwargs
    )
...
def test_plan_step_scales_grid_action_to_pixels() -> None:
    reply = json.dumps(
        {
            "thought": "Ich klicke das Menü.",
            "action": {"type": "click", "x": 500, "y": 250},
            "done": False,
        }
    )
...
def test_plan_step_unknown_action_degrades_to_wait() -> None:
    router = _FakeRouter(
        json.dumps({"thought": "?", "action": {"type": "launch_nukes"}, "done": False})
    )
...
def test_plan_step_done_marks_session_finished() -> None:
    router = _FakeRouter(
        json.dumps({"thought": "fertig", "action": {"type": "done"}, "done": True})
    )
    session = _session()
...
def test_plan_step_enforces_step_budget() -> None:
    router = _FakeRouter(
        json.dumps({"thought": "x", "action": {"type": "wait"}, "done": False})
    )
...
def test_type_and_key_actions_pass_through() -> None:
    reply = json.dumps(
        {"thought": "tippe", "action": {"type": "type", "text": "hallo"}, "done": False}
    )
...
def _plan_reply(
    ...
    return json.dumps(
        {"thought": thought, "action": action, "expectation": expectation, "done": done}
    )
...
def test_verify_success_resets_failure_counter() -> None:
    ...
    verify_ok = json.dumps(
        {"beobachtung": "Menü offen.", "erfuellt": True, "hinweis": ""}
    )
...
def test_forced_ask_after_consecutive_failures() -> None:
    ...
    image = _png_b64(600, 600)
    self_drive.plan_step(
        router, session, image, verify_cfg=_VERIFY_CFG, max_consecutive_failures=1
    )
    result = self_drive.plan_step(
        router, session, image, verify_cfg=_VERIFY_CFG, max_consecutive_failures=1
    )
...
def test_second_stall_fails_terminally() -> None:
    ...
    result = self_drive.plan_step(
        _FakeRouter("{}"), session, _png_b64(400, 400), verify_cfg=_VERIFY_CFG
    )
    assert result["action"]["type"] == "fail"
    assert result["done"] is True
    assert session.finished is True
...
def test_stall_on_repeated_identical_clicks() -> None:
    ...
    session.recent_actions = [
        {"type": "click", "x": 100.0, "y": 100.0} for _ in range(3)
    ]
    router = _FakeRouter("{}")
    result = self_drive.plan_step(router, session, _png_b64(400, 400))
    assert result["action"]["type"] == "ask"
...
def test_refine_overwrites_click_coords() -> None:
    ...
    result = self_drive.plan_step(
        router, _session(), _png_b64(1000, 1000), refine_cfg=_REFINE_CFG
    )
...
def test_lookup_passes_through_when_enabled() -> None:
    plan = _plan_reply(
        {"type": "lookup", "query": "Wie öffnet man in GIMP Ebenen?"}, expectation=""
    )
    result = self_drive.plan_step(
        _FakeRouter(plan), _session(), _png_b64(400, 400), lookup_cfg=_LOOKUP_CFG
    )
    assert result["action"] == {
        "type": "lookup",
        "query": "Wie öffnet man in GIMP Ebenen?",
    }
    assert result["done"] is False
...
def test_lookup_budget_exhausted_degrades_to_wait() -> None:
    ...
    result = self_drive.plan_step(
        _FakeRouter(plan), session, _png_b64(400, 400), lookup_cfg=_LOOKUP_CFG
    )
    assert result["action"] == {"type": "wait"}
...
def test_ask_action_passes_through() -> None:
    plan = _plan_reply(
        {"type": "ask", "question": "Welche Datei meinst du?"}, expectation=""
    )
    result = self_drive.plan_step(_FakeRouter(plan), _session(), _png_b64(400, 400))
...
def test_inject_lookup_result_and_user_answer() -> None:
    ...
    assert (
        session.history[-1]["content"] == "Antwort des Nutzers: Nimm die zweite Datei."
    )
...
def test_classify_sensitive_english_buy_button() -> None:
    sensitive, reason = self_drive.classify_sensitive(
        {"type": "click", "label": "Buy now button"}
    )
    assert sensitive is True
...
def test_classify_sensitive_respects_word_boundaries() -> None:
    ...
    sensitive, _ = self_drive.classify_sensitive(
        {"type": "click", "label": "Buyers Guide öffnen"}
    )
...
def test_plan_step_harmless_action_unflagged() -> None:
    plan = _plan_reply(
        {"type": "click", "x": 500, "y": 250, "label": "Startmenü"}, thought="klick"
    )
...
def test_selfdrive_start_step_stop_flow(monkeypatch) -> None:
    ...
    step = client.post(
        "/selfdrive/step",
        json={"session_id": session_id, "image_base64": _png_b64(500, 500)},
    ).json()
...
def test_selfdrive_step_unknown_session(monkeypatch) -> None:
    ...
    res = client.post(
        "/selfdrive/step",
        json={"session_id": "nope", "image_base64": _png_b64(100, 100)},
    )
