...
def _hit(pid: str, score: float, citations: int, year: int) -> SearchHit:
    hit = SearchHit(
        source=Source(paper_id=pid, title=pid, year=year, citation_count=citations)
    )
...
def test_comparable_relevance_prefers_cited_and_recent() -> None:
    ...
    assert effective_hit_score(cited_recent, now_year=NOW) > effective_hit_score(
        rare_old, now_year=NOW
    )
...
def test_relevance_still_dominates_the_nudge() -> None:
    ...
    assert effective_hit_score(better_match, now_year=NOW) > effective_hit_score(
        cited_recent, now_year=NOW
    )


def test_bonus_is_capped_and_old_papers_are_not_penalised() -> None:
    weights_cap = 1.5  # config default max_bonus
    huge = _citation_recency_bonus(
        Source(paper_id="x", citation_count=10**6, year=2026), now_year=NOW
    )
    assert huge <= weights_cap + 1e-9

    old_unc = _citation_recency_bonus(
        Source(paper_id="y", citation_count=0, year=1990), now_year=NOW
    )
