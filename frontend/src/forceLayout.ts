 ...
 * mehrsekündigen Läufen führen. Gehört in einen Web Worker, nicht auf den
 * Main-Thread.
 *
 ...
 *
 * `pinned` ist der Zusatz für ausklappbare Graphen: bereits platzierte Knoten
 * starten an ihrer alten Stelle und bewegen sich nur gedämpft. Ohne das würde
 * jedes Ausklappen das ganze Bild neu würfeln — und mit dem Bild die Vorstellung,
 * die sich der Betrachter gerade davon gemacht hat.
 */
...
export type LayoutOptions = {
  /** Vorhandene Positionen; diese Knoten bewegen sich nur mit `pinStrength`. */
  pinned?: Map<string, LayoutPoint>;
  /** Dämpfung für gepinnte Knoten (0 = unbeweglich, 1 = frei). */
  pinStrength?: number;
};
...
export function computeForceLayout(
  ...
  const pinStrength = options.pinStrength ?? 0.15;

  // Start auf einer Spirale (deterministisch → stabiles Layout pro Datensatz);
  // bekannte Knoten starten dort, wo sie schon liegen.
  const pos: LayoutPoint[] = nodes.map((node, i) => {
    const known = pinned?.get(node.id);
  ...
  const damping = nodes.map((node) => (pinned?.has(node.id) ? pinStrength : 1));
  ...
  for (let step = 0; step < iterations; step++) {
    const disp: LayoutPoint[] = pos.map(() => ({ x: 0, y: 0 }));
    ...
    for (let i = 0; i < n; i++) {
      ...
      const limited = Math.min(length, temperature) * damping[i];
