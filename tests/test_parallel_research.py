...
def test_parallel_session_crud(tmp_path) -> None:
    ...
    db.add_parallel_entry(
        ...
        "assistant",
        "einordnung",
        answer_payload={"answer": "a", "citation_links": []},
    )
    ...
    db.update_parallel_session(
        sid,
        status="synthesized",
        synthesis_markdown="## Beste",
        synthesis_payload={"answer": "final"},
    )
    ...
    router = _FakeRouter(
        {
            "variants": [
                {
                    "name": "V1",
                    "approach": "tu A [arxiv:1]",
                    "rationale": "weil [arxiv:1]",
                    "prompt": "mach A",
                },
                {
                    "name": "V2",
...
def test_propose_stages_parses_and_caps() -> None:
    retriever = _FakeRetriever([_hit()])
    router = _FakeRouter(
        {"stages": [{"name": f"E{i}", "goal": f"G{i}"} for i in range(8)]}
    )
...
def test_propose_stages_tolerates_bad_llm_output() -> None:
    retriever = _FakeRetriever([_hit()])
    assert (
        parallel_research.propose_stages(retriever, _FakeRouter({"nope": 1}), "Frage")
        == []
    )
...
def test_professor_review_stage_maps_verdicts() -> None:
    ...
    payload = parallel_research.professor_review_stage(
        ...
        router,
        question="F",
        stage={"id": "s1", "name": "Etappe 1", "goal": "G"},
        variants=variants,
    )
    pr = payload["professor_review"]
            ...
            if '"gesamtverstaendnis"' in user:  # synthesize (final review)
                return {
                    ...
                    "etappen_zusammenfassung": [
                        {"stage_id": "?", "name": "Etappe A", "fazit": "Ziel erreicht."}
                    ],
                    "staerken": ["solide"],
                    "probleme": ["offen"],
                    "ideen": ["mehr"],
...
def test_parallel_endpoints_full_flow(client, tmp_path) -> None:
    ...
    res = client.post(
        ...
        json={"question": "Wie X bauen?", "variant_count": 2, **base},
    )
    ...
    res = client.post(
        f"/parallel/variants/{vid}/entries",
        json={"content": "80% erreicht", "request_feedback": True, **base},
    )
    assert res.status_code == 200
    body = res.json()
    ...
    assert (
        client.get(
            "/projects/proj/parallel", params={"metadata_db_path": db_path}
        ).json()["sessions"][0]["stage_count"]
        == 2
    )
...
def test_parallel_stage_endpoints(client, tmp_path) -> None:
    ...
    session = client.post(
        "/projects/proj/parallel",
        json={"question": "Wie X bauen?", "variant_count": 1, **base},
    ).json()["session"]
    sid = session["id"]
    ...
    res = client.post(
        f"/parallel/{sid}/stages", json={"name": "Etappe C", "goal": "Z", **base}
    )
    assert [s["name"] for s in res.json()["session"]["stages"]] == [
        "Etappe A",
        "Etappe B",
        "Etappe C",
    ]
    ...
    res = client.patch(
        f"/parallel/stages/{s1['id']}",
        json={"status": "abgeschlossen", "metadata_db_path": db_path},
    )
    stages = res.json()["session"]["stages"]
    ...
    assert (
        client.patch(
            "/parallel/stages/nope", json={"name": "x", "metadata_db_path": db_path}
        ).status_code
        == 404
    )
    ...
    res = client.post(
        ...
        json={"variant_count": 2, "stage_id": s2["id"], **base},
    )
    ...
    stages = client.get(
        f"/parallel/{sid}", params={"metadata_db_path": db_path}
    ).json()["session"]["stages"]
    for stage in stages[:-1]:
        assert (
            client.delete(
                f"/parallel/stages/{stage['id']}", params={"metadata_db_path": db_path}
            ).json()["deleted"]
            is True
        )
    res = client.delete(
        f"/parallel/stages/{stages[-1]['id']}", params={"metadata_db_path": db_path}
    )
...
def test_parallel_followup_keeps_session_and_appends(client, tmp_path) -> None:
    ...
    res = client.post(
        f"/parallel/{sid}/ask",
        json={"question": "Unkonventionelle Wege?", "variant_count": 1, **base},
    )
    assert res.status_code == 200
    body = res.json()
