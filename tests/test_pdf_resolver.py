...
def test_pdf_resolver_prefers_existing_local_pdf(tmp_path: Path) -> None:
    ...
    result = resolver.resolve(
        paper_id="paper-p1", title="Local Fixture", download_missing=False
    )
...
def test_pdf_resolver_downloads_mocked_open_pdf_with_provenance(
    ...
    monkeypatch.setattr(
        resolver, "_download_pdf", lambda url: (b"%PDF-1.4\nfixture", "application/pdf")
    )
    monkeypatch.setattr(resolver, "_throttle", lambda source: None)

    result = resolver.resolve(
        paper_id="arxiv:2501.00001", title="Open Fixture", download_missing=True
    )
