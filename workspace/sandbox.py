...
def create_worktree(root: Path, checkpoint_sha: str, sandbox_id: str) -> dict[str, Any]:
    ...
    code, _out, err = _run_git(
        root, ["worktree", "add", "--detach", str(target), checkpoint_sha]
    )
    if code != 0:
...
def run_tests(
    worktree: Path, command: list[str], *, timeout: int = DEFAULT_TIMEOUT
) -> SandboxRunResult:
...
def diff_worktree(root: Path, checkpoint_sha: str) -> dict[str, Any]:
    ...
    return {
        "available": True,
        "note": "siehe /workspaces/{id}/git/diff im Sandbox-Projekt",
    }
...
def apply_to_main(
    ...
    *,
    ...
    # Vorher sichern.
    checkpoints.create(
        main_root, label=label, reason="auto_sandbox_apply", author_env=author_env
    )
    ...
    code, out, _ = _run_git(
        worktree, ["diff", "--cached", "--name-status", checkpoint_sha]
    )
    if code != 0:
    ...
    for line in out.splitlines():
        ...
        try:
            ...
            applied.append(
                {"path": rel, "status": status, "action": "skipped", "reason": str(exc)}
            )
