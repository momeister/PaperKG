...
def project_fixture(tmp_path: Path):
        ...
        db.insert_paper(
            {
                ...
                "title": "Attention und Gedaechtnis",
        ...
        db.insert_paper(
            {
                "id": "arxiv:2301.00002",
                "source": "arxiv",
                "source_id": "2301.00002",
                "title": "Vorlaeuferarbeit",
                "abstract": "Noch ein Abstract.",
                "year": 2023,
            }
        )
        db.insert_paper(
            {
                ...
                "title": "Fremdes Paper (nicht im Projekt)",
                "year": 2020,
            }
        )
    ...
    write_json_atomic(
        projects_path, {"Testprojekt": ["arxiv:2401.00001", "arxiv:2301.00002"]}
    )
...
def test_export_contains_only_the_projects_papers(
    tmp_path: Path, project_fixture
) -> None:
    ...
    with zipfile.ZipFile(bundle) as archive:
        ...
        papers = [
            json.loads(line)
            for line in archive.read(TABLE_FILES["papers"]).decode().splitlines()
            if line
        ]
        greys = [
            json.loads(line)
            for line in archive.read(TABLE_FILES["grey_sources"]).decode().splitlines()
            if line
        ]
        embeds = [
            json.loads(line)
            for line in archive.read(TABLE_FILES["entity_embeddings"])
            .decode()
            .splitlines()
            if line
        ]
...
def test_export_can_include_pdfs(tmp_path: Path, project_fixture) -> None:
    ...
    bundle = _export(
        tmp_path, db_path, projects_path, include_pdfs=True, pdf_base_dir=str(pdf_dir)
    )
    with zipfile.ZipFile(bundle) as archive:
...
def test_roundtrip_into_a_fresh_database(tmp_path: Path, project_fixture) -> None:
    ...
    with MetadataDB(str(fresh_db)) as db:
        assert {p["id"] for p in db.list_papers(limit=100)} == {
            "arxiv:2401.00001",
            "arxiv:2301.00002",
        }
        ...
        assert [g["url"] for g in db.list_grey_sources("Testprojekt")] == [
            "https://example.org/a"
        ]
...
def test_importing_twice_creates_no_duplicates(tmp_path: Path, project_fixture) -> None:
    ...
    _report, projects, _ = import_bundle(
        bundle, metadata_db_path=str(fresh_db), existing_projects={}
    )
    second, projects, _ = import_bundle(
        bundle, metadata_db_path=str(fresh_db), existing_projects=projects
    )
...
def test_merge_keeps_local_papers_and_adds_the_new_ones(
    tmp_path: Path, project_fixture
) -> None:
    db_path, projects_path = project_fixture
    bundle = _export(tmp_path, db_path, projects_path)
        ...
        db.insert_paper(
            {
                ...
                "source": "arxiv",
                "source_id": "2401.00001",
                "title": "Lokal korrigierter Titel",
                "year": 2024,
            }
        )
...
def test_import_into_a_differently_named_project(
    tmp_path: Path, project_fixture
) -> None:
    db_path, projects_path = project_fixture
    bundle = _export(tmp_path, db_path, projects_path)
...
def _malicious_zip(path: Path, member: str) -> Path:
    with zipfile.ZipFile(path, "w") as archive:
        archive.writestr(
            MANIFEST_NAME, json.dumps({"bundle_version": 1, "project": "X"})
        )
...
def test_a_non_zip_file_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(BundleError, match="kein gueltiges ZIP"):
        preview_bundle(
            fake, metadata_db_path=str(tmp_path / "db.duckdb"), existing_projects={}
        )
...
def test_a_zip_without_manifest_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(BundleError, match="Kein manifest.json"):
        preview_bundle(
            plain, metadata_db_path=str(tmp_path / "db.duckdb"), existing_projects={}
        )
...
def test_a_newer_bundle_version_is_refused(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(future, "w") as archive:
        archive.writestr(
            MANIFEST_NAME, json.dumps({"bundle_version": 99, "project": "X"})
        )
    with pytest.raises(BundleError, match="neueren Version"):
        preview_bundle(
            future, metadata_db_path=str(tmp_path / "db.duckdb"), existing_projects={}
        )
...
def test_export_and_import_over_http(tmp_path: Path, project_fixture) -> None:
    ...
    with TestClient(product_main.app) as client:
        ...
        target = {
            "metadata_db_path": str(target_db),
            "projects_path": str(target_projects),
        }
        ...
        imported = client.post(
            "/bundles/import", params={**target, "mode": "merge"}, content=payload
        )
    ...
    assert read_json_dict(target_projects)["Testprojekt"] == [
        "arxiv:2401.00001",
        "arxiv:2301.00002",
    ]
