...
This is the **optional automation channel** for the Workspace *Parallelmodus* feature
**„An Desktop-Agent übergeben"**. PaperKG stays the *brain* (it knows your research
question, the chosen variant and the grounded literature, and compiles a precise **task
brief**). This bridge is the *eyes/hands*: it runs the brief on **this machine** using the
[UI-TARS](https://github.com/bytedance/UI-TARS-desktop) GUI agent — it screenshots the
screen, reasons with a **local VLM**, and drives mouse/keyboard.

> ⚠️ **This actually controls your computer.** Run it only on your own machine, keep it in
> view, and be ready to stop it. PaperKG itself never drives the machine — it only hands
> over the text brief.
...
this bridge only if you want PaperKG to dispatch the brief automatically and stream the
agent's progress back into the variant.
...
Restart the product API. In the Workspace Parallelmodus → *Ergebnisse* tab, each variant
now shows **„An Desktop-Agent übergeben"**; with the bridge enabled you also get **„Jetzt
ausführen"**, which POSTs the brief here and streams progress back as an assistant entry.

## How it talks to PaperKG

- PaperKG `POST {url}` with `{"task": "<plain-text brief>"}`.
- The bridge replies with `text/event-stream`; each event is `data: {json}` where `json`
  is `{status:"started|step|done|error", ...}`. PaperKG forwards these to the frontend.

## Env vars

| Var | Default | Meaning |
|---|---|---|
| `BRIDGE_PORT` | `8787` | Port (match `agent_bridge.url`) |
| `VLM_BASE_URL` | `http://127.0.0.1:1234/v1` | OpenAI-compatible VLM endpoint |
| `VLM_API_KEY` | `lm-studio` | API key (LM Studio ignores it) |
| `VLM_MODEL` | `ui-tars-1.5-7b` | Model name as served |
