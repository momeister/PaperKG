...
def _reconstruct_page_text(words: list[dict[str, Any]], page_width: float, page_height: float) -> str | None:
	"""
	...
	if gutter_x is None:
		return None
	header, left, right, footer = _classify_and_split_words(words, gutter_x, page_height)
	if not left or not right:
		return None
	parts = [" ".join(str(w.get("text", "")) for w in group) for group in (header, left, right, footer) if group]
	return "\n\n".join(parts)
