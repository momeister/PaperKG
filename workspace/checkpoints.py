...
from workspace.manager import _run_git, git_available, is_git_repo  # noqa: PLC2701 — privater Helfer, bewusst geteilt
...
def create(
	...
	*,
	...
	try:
		...
		if code != 0:
			return CheckpointResult(None, commit_sha, tree_sha, parent, 0, "error", err.strip())

		file_count = _count_tree_entries(root, tree_sha)
		return CheckpointResult(ref_name, commit_sha, tree_sha, parent, file_count, reason, label=label)
	finally:
		try:
			os.unlink(index_path)
		except OSError:
...
def diff(root: Path, commit_sha: str) -> dict[str, Any]:
	...
	try:
		...
		now_tree = out.strip()
		code, out, _ = _run_git(root, ["diff-tree", "-r", "--name-status", commit_sha, now_tree])
		if code != 0:
...
def restore(
	...
	*,
	...
	if plan.get("plan_hash") != plan_hash:
		return {"applied": False, "reason": "stale", "expected": plan_hash, "got": plan_hash}
	...
	for entry in entries:
		...
		try:
			...
			applied.append({"path": rel, "status": status, "action": "skipped", "reason": "escape"})
			continue
		if status == "A":
			...
			if target.is_file() or target.is_symlink():
				...
				applied.append({"path": rel, "status": status, "action": "skipped", "reason": "gone"})
		else:
			...
			if code == 0:
				...
				applied.append({"path": rel, "status": status, "action": "failed", "reason": err.strip()})
