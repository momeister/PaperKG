...
def export_project(
    ...
    *,
    ...
    if not member_set:
        raise BundleError(
            f"Projekt „{project_id}“ enthaelt keine Paper — nichts zu exportieren."
        )

    output_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now()
    target = output_dir / bundle_filename(
        project_id, timestamp.strftime("%Y-%m-%d_%H%M%S")
    )

    with MetadataDB(metadata_db_path) as db:
        papers = [
            p
            for p in db.list_papers(limit=_ROW_LIMIT)
            if str(p.get("id") or "") in member_set
        ]
        found_ids = {str(p.get("id") or "") for p in papers}
        extractions = [
            e
            for e in db.list_extraction_results(limit=_ROW_LIMIT)
    ...
    if missing:
        logger.info(
            "Export %s: %d Projekt-Paper ohne DB-Eintrag uebersprungen",
            project_id,
            len(missing),
        )
...
def _paper_sources(db: MetadataDB, paper_ids: set[str]) -> list[dict[str, Any]]:
    ...
    if not paper_ids:
        return []
    rows = db._execute(
        "SELECT paper_id, source, source_id, source_url FROM paper_sources"
    ).fetchall()
    return [
...
def _relevant_embeddings(
    db: MetadataDB, extractions: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    return [
        ...
        if str(row.get("label_norm") or row.get("label") or "").strip().lower()
        in wanted
    ]


def _add_pdfs(
    archive: zipfile.ZipFile, papers: list[dict[str, Any]], pdf_base_dir: str | None
) -> int:
