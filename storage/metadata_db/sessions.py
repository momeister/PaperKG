...
class SessionsMixin(_Base):
    ...
    def save_workspace_session(
        ...
        previous_turns = self._workspace_turn_count(
            current.get("payload") if current else None
        )
        ...
        return self.get_workspace_session(project_id) or {
            "project_id": str(project_id),
            "payload": {},
        }
    ...
    def _backup_workspace_session(
        ...
        self._execute(
            ...
            [
                str(project_id),
                saved_at,
                json.dumps(session.get("payload") or {}),
                int(turn_count),
            ],
        )
        self._execute(
    ...
    def restore_workspace_session(
        self, project_id: str, saved_at: Any = None
    ) -> dict[str, Any] | None:
    ...
    def list_research_sessions(
        ...
        for row in rows:
            ...
            rec["done_count"] = sum(
                1 for n in nodes if isinstance(n, dict) and n.get("status") == "done"
            )
            rec["has_synthesis"] = any(
                isinstance(n, dict) and n.get("status") == "synthesis" for n in nodes
            )
            out.append(rec)
        return out
    ...
    def get_parallel_session(self, session_id: str) -> dict[str, Any] | None:
        row = self._execute(
            "SELECT * FROM parallel_sessions WHERE id = ?", [str(session_id)]
        ).fetchone()
        if row is None:
            return None
        cols = [desc[0] for desc in self.conn.description]
        session = dict(zip(cols, row))
        session["synthesis_payload"] = self._decode_json(
            session.get("synthesis_payload"), None
        )
        session["overview_payload"] = self._decode_json(
            session.get("overview_payload"), None
        )
        ...
        for entry in self.list_parallel_entries(session_id=session_id):
            entries_by_variant.setdefault(str(entry.get("variant_id")), []).append(
                entry
            )
    ...
    def list_parallel_sessions(
        ...
        for row in rows:
            rec = dict(zip(cols, row))
            count = self._execute(
                "SELECT COUNT(*) FROM parallel_variants WHERE session_id = ?",
                [rec["id"]],
            ).fetchone()
    ...
    def update_parallel_session(
        ...
        next_md = (
            ...
            else current.get("synthesis_markdown")
        )
        next_payload = (
            synthesis_payload
            if synthesis_payload is not None
            else current.get("synthesis_payload")
        )
        next_overview_md = (
            overview_markdown
            if overview_markdown is not None
            else current.get("overview_markdown")
        )
        next_overview_payload = (
            overview_payload
            if overview_payload is not None
            else current.get("overview_payload")
        )
        ...
        """,
            [
                ...
                next_overview_md,
                (
                    json.dumps(next_overview_payload)
                    if next_overview_payload is not None
                    else None
                ),
                next_task_id,
    ...
    def delete_parallel_session(self, session_id: str) -> bool:
        if self.get_parallel_session(session_id) is None:
            return False
        self._execute(
            "DELETE FROM parallel_followups WHERE session_id = ?", [str(session_id)]
        )
        self._execute(
            "DELETE FROM parallel_entries WHERE session_id = ?", [str(session_id)]
        )
        self._execute(
            "DELETE FROM parallel_variants WHERE session_id = ?", [str(session_id)]
        )
        self._execute(
            "DELETE FROM parallel_stages WHERE session_id = ?", [str(session_id)]
        )
    ...
    def get_parallel_stage(self, stage_id: str) -> dict[str, Any] | None:
        row = self._execute(
            "SELECT * FROM parallel_stages WHERE id = ?", [str(stage_id)]
        ).fetchone()
        if row is None:
            return None
        cols = [desc[0] for desc in self.conn.description]
    ...
    def delete_parallel_stage(self, stage_id: str) -> bool:
        ...
        remaining = [
            s
            for s in self.list_parallel_stages(session_id)
    ...
    def add_parallel_variant(
        ...
        """,
            [
                ...
                str(name or "Variante"),
                str(approach or ""),
                str(rationale or ""),
                str(suggested_prompt or ""),
                str(origin or "ai"),
                str(status or "vorgeschlagen"),
                int(position),
    ...
    def get_parallel_variant(self, variant_id: str) -> dict[str, Any] | None:
        row = self._execute(
            "SELECT * FROM parallel_variants WHERE id = ?", [str(variant_id)]
        ).fetchone()
        if row is None:
            return None
        cols = [desc[0] for desc in self.conn.description]
    ...
    def update_parallel_variant(
        self, variant_id: str, **fields: Any
    ) -> dict[str, Any] | None:
    ...
    def delete_parallel_variant(self, variant_id: str) -> bool:
        ...
        if current is None:
            return False
        self._execute(
            "DELETE FROM parallel_entries WHERE variant_id = ?", [str(variant_id)]
        )
    ...
    def add_parallel_entry(
        ...
        """,
            [
                ...
                str(role or "user"),
                str(content or ""),
                json.dumps(answer_payload) if answer_payload is not None else None,
                now,
            ],
        )
        self._touch_parallel_session(session_id)
        row = self._execute(
            "SELECT * FROM parallel_entries WHERE id = ?", [eid]
        ).fetchone()
        cols = [desc[0] for desc in self.conn.description]
        rec = dict(zip(cols, row))
