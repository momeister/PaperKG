...
def add_project_grey_sources(
    project_id: str, payload: GreySourcePayload
) -> dict[str, Any]:
...
async def add_grey_source_from_url(
    project_id: str, payload: GreySourceFromUrlPayload
) -> dict[str, Any]:
    ...
    if not re.match(r"^https?://", url):
        raise HTTPException(
            status_code=400, detail="Nur http(s)-URLs werden unterstützt"
        )
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Konnte URL nicht laden: {exc}"
        ) from exc
...
def delete_grey_source(
    grey_id: str, metadata_db_path: str = DEFAULT_METADATA_DB_PATH
) -> dict[str, Any]:
    with MetadataDB(metadata_db_path) as db:
