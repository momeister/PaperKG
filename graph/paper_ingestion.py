...
def to_phase2_paper_node(record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "obsolescence_score": float(
            ...
            else compute_obsolescence_score(
                record.get("year"), int(record.get("citation_count") or len(citations))
            )
        ),
...
def ingest_extractions_from_metadata_db(
    ...
    for extraction in metadata_db.list_extraction_results(limit=limit):
        ...
        for concept in _accepted_entities(
            extraction, "concepts", legacy_threshold=0.75
        ):
        ...
        for relation in _accepted_relations(extraction):
            ...
            if not subject_id or not object_id or subject_id == object_id:
                continue
            if (
                subject_id not in current_concept_ids
                or object_id not in current_concept_ids
            ):
                continue
...
def to_concept_node(concept: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not label:
        return None
    openalex_id = (
        concept.get("openAlex_id")
        or concept.get("openalx_id")
        or concept.get("openalex_id")
    )
    concept_id = str(
        concept.get("canonical_id")
        or f"concept:{_stable_id(str(openalex_id or label))}"
    )
    return {
        "id": concept_id,
...
def _extraction_stub_paper(
    raw_paper_id: str, extraction: dict[str, Any]
) -> dict[str, Any]:
...
def _stable_id(value: str) -> str:
    ...
    if slug:
        return slug[:96]
    return hashlib.sha1(normalize_scientific_text(value).encode("utf-8")).hexdigest()[
        :16
    ]
...
def _accepted_entities(
    ...
    if not isinstance(items, list):
        return []
    candidate_field = (
        "concept_candidates" if field == "concepts" else "method_candidates"
    )
    has_candidate_split = (
        candidate_field in extraction and extraction.get(candidate_field) is not None
    )
...
def _metadata_blocks_graph_write(extraction: dict[str, Any]) -> bool:
    ...
    status = str(
        extraction.get("metadata_status") or payload.get("metadata_status") or ""
    ).lower()
    if status == "invalid":
        return True
    blocking_errors = (
        extraction.get("blocking_errors") or payload.get("blocking_errors") or []
    )
...
def ingest_from_metadata_db(
    ...
    if include_extractions:
        semantic_stats = ingest_extractions_from_metadata_db(
            graph, metadata_db, limit=limit
        )
