...
def _register_project(db: MetadataDB, path: Path) -> dict[str, Any]:
	...
	for project in db.list_code_projects():
		if Path(project["path"]).resolve() == path.resolve():
			return project
	return db.add_code_project(name="PaperKG-Prompt-Check", path=str(path), kind="external")


def main() -> int:
	parser = argparse.ArgumentParser(description=__doc__)
	parser.add_argument("--provider", default=None, help="LLM-Provider (sonst config-Default)")
	parser.add_argument("--model", default=None, help="Modellname (sonst Provider-Default)")
	...
	parser.add_argument("--limit", type=int, default=None, help="Nur die ersten N Fragen")
	...
	if not status.get("index_exists") or status.get("status") != "ready":
		...
		for event in service.index(db, project, config_path=args.config):
			if event.get("event") == "progress":
				...
				print(f"Indizierung fehlgeschlagen: {event.get('error')}", file=sys.stderr)
				return 3
	...
	for question in questions:
		...
		try:
			...
			record = {
				...
				"reasoning_truncated": bool(router.last_response_metadata.get("reasoning_truncated")),
		...
		json.dumps(
			{
				...
				"reasoning_truncated": sum(1 for r in results if r.get("reasoning_truncated")),
				"results": results,
			},
			indent=2,
			ensure_ascii=False,
