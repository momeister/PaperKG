...
def _git(root: Path, args: list[str], env: dict[str, str] | None = None) -> tuple[int, str, str]:
	proc = subprocess.run(
		["git", *args],
		cwd=str(root),
		capture_output=True,
...
def test_create_worktree_on_a_checkpoint(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_run_tests_executes_a_trivial_command(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
	...
	result = sandbox.run_tests(Path(created["path"]), ["python", "-c", "import sys; sys.exit(0)"])
	assert result.returncode == 0
	assert not result.timed_out


def test_apply_brings_sandbox_changes_back_to_main(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
