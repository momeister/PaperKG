...
def is_available(base_url: str = DEFAULT_BASE_URL, timeout: float = 2.0) -> bool:
    ...
    try:
        with urllib.request.urlopen(
            f"{base_url}/system_stats", timeout=timeout
        ) as resp:
...
def _default_workflow(prompt: str, checkpoint: str, seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "9": {
            ...
            "inputs": {"filename_prefix": "sciencekg", "images": ["8", 0]},
        },
    }
    ...
    *,
    ...
    try:
        ...
        workflow = _default_workflow(
            prompt, checkpoint, seed=int(time.time()) % 1_000_000
        )
        queued = _post_json(
            f"{base_url}/prompt",
        ...
        while time.time() < deadline:
            try:
                with urllib.request.urlopen(
                    f"{base_url}/history/{prompt_id}", timeout=10.0
                ) as resp:
        ...
        for node_out in outputs.values():
            for image in node_out.get("images") or []:
                ...
                with urllib.request.urlopen(
                    f"{base_url}/view?{params}", timeout=30.0
                ) as img_resp:
